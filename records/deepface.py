...
#from tensorflow.keras import backend K
...
#The model here constructed is based on FaceNet's Inception model customize.

myInput = Input(shape=(128, 128, 3))
...
#FaceNet is trained by minimizing the triplet loss. I will be loading a previously trained model.
# weights are avaiable at https://github.com/iwantooxxoox/Keras-OpenFace in the "weights" folder which is also provided in this source.
...
#This function calculate similarity between the captured image and the images that are already been stored.
...
def create_input_image_embeddings():
    ...
    for file in glob.glob(BASE_DIR+'/ml/dataset/*'):
...
def recognize_faces_in_cam(input_embeddings):
    ...
    face_cascade = cv2.CascadeClassifier(BASE_DIR+'/ml/haarcascade_frontalface_default.xml')
...
#Following code captures 10 face images of the person. They all are stored in "images" folder with the name User_1 to User_10.
# Select a good captured image from the set of 10 images. Rename it with the name of person and delete rest of them.
# This image will be used for recognizing the the identity of the person using one shot learning.

cam = cv2.VideoCapture(0)

face_detector = cv2.CascadeClassifier(BASE_DIR+'/ml/haarcascade_frontalface_default.xml')
